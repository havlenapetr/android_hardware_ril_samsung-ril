...
void ipc2ril_card_status(struct ipc_sec_pin_status_response *pin_status, RIL_CardStatus_v5 *card_status)
{
...
void ipc_sec_pin_status(struct ipc_message_info *info)
{
	...
	switch(info->type) {
		...
		case IPC_TYPE_RESP:
			...
			RIL_onRequestComplete(t, RIL_E_SUCCESS, &card_status, sizeof(RIL_CardStatus_v5));
...
void ril_request_sim_io(RIL_Token t, void *data, size_t datalen)
{
	...
	sim_io = (const RIL_SIM_IO_v5*)data;
 ...
 *   Supplies SIM PIN. Only called if RIL_CardStatus_v5 has RIL_APPSTATE_PIN state
 * 
